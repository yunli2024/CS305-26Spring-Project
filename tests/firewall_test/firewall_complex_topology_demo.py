...
class SevenSwitchFirewallTopo(Topo):
    def build(self):
        hosts = {
            "h1": "192.168.117.2/24",
            "h2": "192.168.117.3/24",
...
def run_mininet():
    ...
    h4.cmd('pkill -f "python3 -m http.server" || true')
    ...
    h4.cmd("python3 -m http.server 8081 --bind 192.168.117.5 >/tmp/h4-http8081.log 2>&1 &")
    time.sleep(1)

    tests = []

    received, output = ping_result(h1, "192.168.117.3")
    tests.append(("h1 -> h2 ICMP is blocked", received == 0, output))
    ...
    passed, output = wait_for_ping(h3, "192.168.117.5")
    tests.append(("h3 -> h4 ICMP remains reachable", passed, output))

    passed, output = wait_for_curl(h6, "http://192.168.117.5:8081/")
    tests.append(("h6 -> h4 TCP/8081 remains reachable", passed, output))

    passed, output = wait_for_ping(h7, "192.168.117.6")
    tests.append(("h7 -> h5 ICMP remains reachable", passed, output))

    passed, output = wait_for_curl(h2, "http://192.168.117.2:8081/")
    tests.append(("Reverse h2 -> h1 TCP/8081 is allowed", passed, output))

    print("\n===== Complex Firewall Demo Results =====")
